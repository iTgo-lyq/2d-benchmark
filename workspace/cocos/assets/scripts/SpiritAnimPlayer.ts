...
import { _decorator, Component, Node, assetManager as R, JsonAsset, SpriteFrame, Sprite, Texture2D, tween, Vec3, Quat, ImageAsset } from 'cc';
                ...
                for (const key in spirit) {
                    ...
                    for (let i = 0; i < spirit[key].length; i++) {
                        ...
                        R.loadRemote(`${BASE_URL}/spirit/${key}/${filename}`, ImageAsset, (err, data: ImageAsset) => {
                            ...
                            tween(node)
                                .repeatForever(
                                    tween().to(.3, { scale: new Vec3(1.25, 1.25, 1.25) }, { easing: "backOut" })
                                        .to(.3, { scale: new Vec3(1, 1, 1) }, { easing: "backIn" })
                                )
                                .start()
